...
def main(args):
    ...
    data_list = dataset[args.judge_model].values
    # load prompt
    ...
    for idx, data in enumerate(tqdm(data_list)): 
        ...
        try:
            ...
            if response == 1:
                ...
                df.to_csv(f"{args.save_path}/{args.judge_model}_{args.model}_temporal.csv")
        except:
    ...
    df = pd.DataFrame({"pred": generated_list, "data": data_list}, index = dataset.index)
    df.to_csv(f"{args.save_path}/{args.judge_model}_{args.model}.csv")

    pred_list = []
    for pred in df['pred'].values:
        pred_list.append(list(json.loads(pred).values())[0])

    nov_score = sum(pred_list) / len(pred_list)
    print(f"{args.judge_model}, {nov_score}")
...
def parse_args():
    ...
    parser.add_argument("--judge_model", default="LED", type=str, choices=['BART', 'PEGASUS','LED', "LLAMA2", "GPT"])
    parser.add_argument("--save_path", default="output/nov_eval_res", type=str)
    parser.add_argument("--data_dir", default="outputs/novelty_judge_samples.csv", type=str)
