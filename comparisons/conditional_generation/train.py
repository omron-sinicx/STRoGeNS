...
from dataloader import conf_bart
from dataloader import conf_bart_shuffle
...
def main(args):
    ...
    # creating model
    if args.resume:
        print(f"{args.resume}/{args.ckpt_best}")
        model = AutoModelForSeq2SeqLM.from_pretrained(f"{args.resume}/{args.ckpt_best}")
        max_input_length = model.config.max_position_embeddings
        model.enable_input_require_grads()
        model.gradient_checkpointing_enable()
    else:
        model = AutoModelForSeq2SeqLM.from_pretrained(args.model, cache_dir=args.cache_dir, ignore_mismatched_sizes=True, max_position_embeddings=args.max_token_length)
        max_input_length = model.config.max_position_embeddings
    ...
    # dataset
    if args.shuffle:
        train_dataloader, eval_dataloader, test_dataloader = conf_bart_shuffle(
            args.data_dir, tokenizer, args.batch_size, max_input_length
        )
    else:
        train_dataloader, eval_dataloader, test_dataloader = conf_bart(
            args.data_dir, tokenizer, args.batch_size, max_input_length
        )
...
def parse_args():
    ...
    parser.add_argument("--data_dir", default="./hg_dataset_new/conf_rw", type=str)
